...
## Guide to outputs
---
MetaPro produces many outputs for the user to use.  They can be found in the outputs -> final_results directory
The following is a guide to the information each file contains 
- Gene map
...
- Cytoscape Network
This table is the import table to CytoScape.  It will display enzyme information, expressed in RPKM of the supplied taxa
-- The EC label that is being represented
-- The total RPKM
-- The RPKM, broken down by taxa representing at least 1% (on default)
-- The piechart display information

## Guide to other outputs
---
MetaPro produces interim files that are then constructed into reports we think they user will likely want.  However, we cannot predict every use-case.  Therefore, this section describes the outputs of the intermediate steps, so that the user may be able to use MetaPro's outputs for their own needs.

Each step of MetaPro will contain a directory called "final_results".  This folder contains the final outputs of the step.

+ Quality Filter
Depending on if the input data was single-ended or paired-ended, MetaPro will produce different files
-- singletons.fastq (The remaining reads after they have been filtered by the quality-filter stage)
-- singletons.unique.fastq.clstr
