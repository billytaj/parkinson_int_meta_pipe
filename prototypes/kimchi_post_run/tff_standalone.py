...
def find_common_ancestry(sample_name, ref_dict, sample_list, return_dict):
    ...
    for ref_key in ref_dict:
        ...
        print("==============================")
        print("Size of sample:", sample_name, sample_size)
        print(sample_list)
        print("size of ref:", ref_key, ref_size)
        print(ref_list)
        ...
        for i in range(0, range_of_loop):
            ...
            if(ref_list[i] == sample_list[i]):
                ...
                print("match! for ref:", ref_key, sample_name,  i, ref_list[i], sample_list[i])
            else:
                print("no match at ", i, ref_key, sample_name, ref_list[i], sample_list[i])
                print("not common at:", search_depth, "breaking")
                search_depth -= 1
                break
        #print(sample_name, "last ancestor against", ref_key, "->", )
        #search_depth -= 1
        
        print("last match for ", ref_key, search_depth)
        if(search_depth > prior_depth):        
            return_dict[sample_name] = common_ancestor
            prior_depth = search_depth
            print("new prior depth for ", ref_key, prior_depth)
        else:
            print("search depth:", search_depth, "prior:", prior_depth)
            print("not updating")
    print(return_dict)
