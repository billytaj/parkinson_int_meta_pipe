...
def prot_map(hits):                         # fail-mapped contig/readIDs=
    ...
    # return unmapped set:
    return unmapped, mapped_reads, prot2read_map
...
def filter_consumed_reads(read_file, DMD_tab_file, output_file):    
    if(not os.path.exists(read_file)):
        print("file not found:", read_file)
        return None
    if(not os.path.exists(DMD_tab_file)):
        print("file not found:", DMD_tab_file)
        return None
    ...
    unmapped_reads, mapped_reads, prot2read_map = prot_map(DMD_hits)                  # Store DMD-aligned contigs/reads in prot2read_map
