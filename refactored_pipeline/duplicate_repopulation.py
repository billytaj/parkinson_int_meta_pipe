...
def repopulate_double(ref_1_filename, ref_2_filename, mRNA_1_filename, mRNA_2_filename, cluster_1_filename, cluster_2_filename):
    ...
    ref_2_file = pd.read_csv(ref_2_filename, header = None, names = [None], sep = '\n', skip_blank_lines = False)
    ref_2_df = pd.DataFrame(ref_1_file.values.reshape(int(len(ref_1_file)/4), 4))
    ref_2_df.columns = ["ID", "seq", "junk", "quality"]
'''
