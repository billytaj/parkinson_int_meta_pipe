...
def import_taxa_info(read2taxid):
...
def separate_parent_child_taxa(taxid_count, rank_taxid):
...
def filter_child_taxa(child_list_orig, taxid_count_orig):
    ...
    child_list = child_list_orig
    taxid_count = taxid_count_orig
    if ID_list == "None":
        for taxid in taxid_count:
...
if __name__ == "__main__":
    ...
    taxa_limit = sys.argv[9]                #IN: string: supposed to limit the taxa displayed for cytoscape
    ...
    nodes_dict = import_nodes_dict(nodes)
    names_dict = import_names_dict(names)
    
    gene2read_dict = import_gene_map(gene2read)
    EC2genes_dict = import_ec_map(gene2EC)
    
    read2taxid_dict, cutoff_count taxid_count = import_taxa_info(read2taxid)

    parent_list, child_list = separate_parent_child_taxa(taxid_count, rank_taxid)
    child_list, taxid_count = filter_child_taxa(child_list, taxid_count)
