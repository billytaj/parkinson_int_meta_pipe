...
def import_taxa(read2taxid, cutoff):
    ...
    return read2taxid_dict, cutoff_count, taxid_count
    
    
def generate_lineage(taxid_count, rank_taxid, nodes_dict):
...
def combine_families(taxid_count, child_list, parent_list):    
    # adds child nodes to parent nodes if they do not meet cutoff or are not in ID list
...
def update_read2taxid_dict(read2taxid_dict, taxid_count, parent_list):
...
def sort_by_parent(rank_taxid, parent_list):
...
def import_EC(gene2EC):
    ...
    return EC2genes_dict
    
def import_genes(gene2read):
    ...
    return gene2read_dict, mapped_reads
    
    
def make_RPKM(gene2read_dict, mapped_reads):
...
if __name__ == "__main__":
    ...
    EC2genes_dict = import_EC(gene2EC)
    nodes_dict = import_nodes(nodes)
    names_dict = import_names(names)
    gene2read_dict, mapped_reads = import_genes(gene2read)

    read2taxid_dict, cutoff_count, taxid_count = import_taxa(read2taxid, cutoff)

    parent_list, child_list = generate_lineage(taxid_count, rank_taxid, nodes_dict)
    combine_families(taxid_count, child_list, parent_list)
    update_read2taxid_dict(read2taxid_dict, taxid_count, parent_list)

    sort_by_parent(rank_taxid, parent_list)
    

    # generating rank names
    ...
    RPKM_dict = make_RPKM(gene2read_dict, mapped_reads)
