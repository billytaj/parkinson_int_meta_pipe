...
def export_leftover_paired_reads_v2(paired_gene_map, paired_df, export_path): 
    #extracts the reads from the gene map, and exports
    read_list = []
    for item in paired_gene_map:
        read_list += paired_gene_map[item][2:]
        
    paired_leftover = paired_df[~paired_df["ID"].isin(read_list)]
    paired_export_path = os.path.join(export_path, "GA_leftover_paired.fastq")
    paired_leftover.to_csv(paired_export_path, mode = "w", sep = "\n", header = False, index = False, quoting = 3)
...
def final_gene_map_merge(gene_map_0, gene_map_1):
    #simple merge.  we don't expect reads to duplicate across categories
...
if __name__ == "__main__":
    ...
    if(operating_mode == "single"):
        ...
        paired_raw_df = import_fastq(os.path.join(assemble_path, "paired_merged.fastq"))
        #pair_2_raw_df = import_fastq(os.path.join(assemble_path, "pair_2.fastq"))
        
        manager = mp.Manager()
        mgr_bwa_paired_gene_map     = manager.dict()
        mgr_bwa_singletons_gene_map = manager.dict()
        mgr_bwa_contig_gene_map     = manager.dict()

        mgr_blat_paired_gene_map    = manager.dict()
        mgr_blat_singletons_gene_map= manager.dict()
        mgr_blat_contig_gene_map    = manager.dict()
        
        mgr_dia_paired_gene_map     = manager.dict()
        mgr_dia_singletons_gene_map = manager.dict()
        mgr_dia_contig_gene_map     = manager.dict()
        ...
        #merge the gene maps, but group them by data context (pair1, pair2, singletons, contigs) for each tool (BWA, BLAT, DIAMOND)
        process = mp.Process(target = concatenate_gene_maps_v2, args = (bwa_path, mgr_bwa_paired_gene_map, "paired"))
        process.start()
        process_store.append(process)
        ...
        #----------------------------------------------------------------------
        process = mp.Process(target = concatenate_gene_maps_v2, args = (blat_path, mgr_blat_paired_gene_map, "paired"))
        process.start()
        process_store.append(process)
        ...
        #----------------------------------------------------------------------
        process = mp.Process(target = concatenate_gene_maps_v2, args = (diamond_path, mgr_dia_paired_gene_map, "paired"))
        process.start()
        process_store.append(process)
        ...
        #-----------------------------------------------------------------------
        #convert to a standard dict
        bwa_paired_gene_map     = dict(mgr_bwa_paired_gene_map)
        bwa_singleton_gene_map  = dict(mgr_bwa_singletons_gene_map)
        bwa_contig_gene_map     = dict(mgr_bwa_contig_gene_map)
        
        blat_paired_gene_map    = dict(mgr_blat_paired_gene_map)
        blat_singleton_gene_map = dict(mgr_blat_singletons_gene_map)
        blat_contig_gene_map    = dict(mgr_blat_contig_gene_map)
        
        dia_paired_gene_map     = dict(mgr_dia_paired_gene_map)
        dia_singleton_gene_map  = dict(mgr_dia_singletons_gene_map)
        dia_contig_gene_map     = dict(mgr_dia_contig_gene_map)
        
        
        paired_gene_map_list    = [bwa_paired_gene_map, blat_paired_gene_map, dia_paired_gene_map]
        singleton_gene_map_list = [bwa_singleton_gene_map, blat_singleton_gene_map, dia_singleton_gene_map]
        contig_gene_map_list    = [bwa_contig_gene_map, blat_contig_gene_map, dia_contig_gene_map]
        ...
        paired_gene_map     = merge_dicts(paired_gene_map_list)
        singletons_gene_map = merge_dicts(singleton_gene_map_list)
        contig_gene_map     = merge_dicts(contig_gene_map_list)
        ...
        export_leftover_paired_reads_v2(paired_gene_map, paired_df, export_path)
