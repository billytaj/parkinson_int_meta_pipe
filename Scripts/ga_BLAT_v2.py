...
def merge_defaultdicts(main_map, mini_map):
    #This just merges 2 defaultdicts together.
...
def filter_consumed_reads(read_file, BLAT_tab_file, output_file):

    if(not os.path.exists(read_file)):
        print("file not found, skipping:", read_file)
        return None
    if(not os.path.exists(BLAT_tab_file)):
        print("file not found, skipping:", BLAT_tab_file)
        return None
    ...
    unmapped_reads, mapped_reads, gene2read_map = gene_map(BLAT_hits)                 # Add BLAT-aligned contigs/reads to gene2read_map
...
if __name__ == "__main__":
    ...
    contigs_fasta = sys.argv[7]
    contigs_blat = sys.argv[8]
    contigs_out = sys.argv[9]
    
    singletons_fasta = sys.argv[10]
    singletons_blat = sys.argv[11]
    singletons_out = sys.argv[12]
    
    pair_1_fasta = sys.argv[13]
    pair_1_blat = sys.argv[14]
    pair_1_out = sys.argv[15]
    
    pair_2_fasta = sys.argv[16]
    pair_2_blat = sys.argv[17]
    pair_2_out = sys.argv[18]
    ...
    BWAreads= []                                        # DEBUG
    BWA_gene2read_map= defaultdict(list)                # Dict of BWA&BLAT-aligned geneID<->readID(s)
    with open(gene2read_file,"r") as mapping:           #  initialized w BWA-alignments.
        for line in mapping:
            if len(line)>5:                             # line at least 5 characeters?
                entry= line.strip("\n").split("\t")
                BWA_gene2read_map[entry[0]]= entry[3:]  # key=geneID, value=list of readID(s)
    ...
    contig_gene2read_map        = filter_consumed_reads(contigs_fasta, contigs_blat, contigs_out)
    singletons_gene2read_map    = filter_consumed_reads(singletons_fasta, singletons_blat, singletons_out)
    pair_1_gene2read_map        = filter_consumed_reads(pair_1_fasta, pair_1_blat, pair_1_out)
    pair_2_gene2read_map        = filter_consumed_reads(pair_2_fasta, pair_2_blat, pair_2_out)
    
    #merge the defaultdicts
    BLAT_gene2read_map = contig_gene2read_map
    BLAT_gene2read_map = merge_defaultdicts(gene2read_map, singletones_gene2read_map)
    BLAT_gene2read_map = merge_defaultdicts(gene2read_map, pair_1_gene2read_map)
    BLAT_gene2read_map = merge_defaultdicts(gene2read_map, pair_2_gene2read_map)
    
    gene2read_map = BWA_gene2read_map
    gene2read_map = merge_defaultdicts(gene2read_map, BLAT_gene2read_map)
    ...
    out_map = open(new_gene2read_file,"w")               # Delete old gene2read_file and write a new one.
        for record in SeqIO.parse(DNA_DB, "fasta"):         # Loop through SeqRec of all genes in DNA db:
            ...
            if (record.id in BLAT_gene2read_map):
