...
# This version lacks the file splitting portion.
# It also, only acts on the split files that are in the input folder.
# This makes it easier to restart after an error.
...
# - Changed JobID_Annotate_Diamond_Post from depend=afterany to depend=afterok
...
#       expectd by the subsequent annotation step.
...
for genome_name in sorted(os.walk(input_folder).next()[1]):
    ...
    for item in sorted(file_list):
        ...
        if startpoint <= sp['Pre'] and endpoint >= sp['Pre']:
            ...
            # REMOVE VECTOR CONTAMINANTS (BWA):
            ...
            # REMOVE VECTOR CONTAMINANTS (BLAT):
